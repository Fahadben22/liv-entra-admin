...
const TIER_COLOR: Record<string, { bg: string; color: string; border: string }> = {
  ...
  basic:        { bg: 'rgba(255,255,255,.04)', color: '#a1a1aa', border: 'rgba(255,255,255,.08)' },
  professional: { bg: 'rgba(99,102,241,.08)', color: '#6366f1', border: 'rgba(99,102,241,.2)' },
...
function Toggle({ on, disabled, onClick }: { on: boolean; disabled?: boolean; onClick: () => void }) {
  return (
    <div onClick={disabled ? undefined : onClick}
      style={{ width: 44, height: 24, borderRadius: 12, background: on ? '#22c55e' : '#3f3f46', position: 'relative', flexShrink: 0, cursor: disabled ? 'default' : 'pointer', opacity: disabled ? 0.5 : 1, transition: 'background .2s' }}>
      <span style={{ position: 'absolute', top: 3, left: on ? 22 : 3, width: 18, height: 18, borderRadius: '50%', background: '#fafafa', transition: 'left .2s', boxShadow: '0 1px 2px rgba(0,0,0,.2)' }} />
    </div>
  );
}
    ...
    <div className="fade-in" style={{ fontFamily: 'system-ui, sans-serif' }}>
      ...
      {noteModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#18181b', borderRadius: 12, padding: 24, width: 400, border: '1px solid rgba(255,255,255,.1)' }}>
            <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 14, color: '#fafafa' }}>ملاحظة على الميزة</div>
            <textarea value={noteText} onChange={e => setNoteText(e.target.value)} rows={4}
              placeholder="سبب التفعيل، اتفاقية مع العميل، وغيرها..."
              style={{ width: '100%', border: '1px solid rgba(255,255,255,.08)', borderRadius: 8, padding: '7px 12px', fontSize: 13, resize: 'vertical', boxSizing: 'border-box', background: 'rgba(255,255,255,.04)', color: '#fafafa' }} />
            <div style={{ display: 'flex', gap: 8, marginTop: 14, justifyContent: 'flex-end' }}>
              <button onClick={() => setNoteModal(null)} style={{ padding: '7px 16px', borderRadius: 7, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.04)', cursor: 'pointer', fontSize: 12, fontWeight: 500, color: '#a1a1aa' }}>إلغاء</button>
              <button onClick={async () => {
                ...
                setNoteModal(null);
              }} style={{ padding: '7px 16px', borderRadius: 7, border: 'none', background: '#6366f1', color: '#fff', cursor: 'pointer', fontSize: 12, fontWeight: 500 }}>حفظ وتفعيل</button>
            </div>
          </div>
        </div>
      )}
      ...
      {rolloutModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#18181b', borderRadius: 12, padding: 24, width: 380, border: '1px solid rgba(255,255,255,.1)' }}>
            <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 6, color: '#fafafa' }}>نسبة الطرح التدريجي</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              ...
              <span style={{ fontWeight: 600, fontSize: 18, color: '#fafafa', width: 50, textAlign: 'center' }}>{rolloutValue}%</span>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              {[25, 50, 75, 100].map(v => (
                <button key={v} onClick={() => setRolloutValue(v)}
                  style={{ flex: 1, padding: '5px 0', borderRadius: 7, border: `1px solid ${rolloutValue === v ? '#6366f1' : 'rgba(255,255,255,.08)'}`, background: rolloutValue === v ? 'rgba(99,102,241,.15)' : 'rgba(255,255,255,.04)', color: rolloutValue === v ? '#6366f1' : '#a1a1aa', fontSize: 12, cursor: 'pointer' }}>
                  {v}%
                </button>
              ))}
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 16, justifyContent: 'flex-end' }}>
              <button onClick={() => setRolloutModal(null)} style={{ padding: '7px 16px', borderRadius: 7, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.04)', cursor: 'pointer', fontSize: 12, fontWeight: 500, color: '#a1a1aa' }}>إلغاء</button>
              <button onClick={async () => {
              ...
              }} style={{ padding: '7px 16px', borderRadius: 7, border: 'none', background: '#6366f1', color: '#fff', cursor: 'pointer', fontSize: 12, fontWeight: 500 }}>تطبيق النسبة</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Header */}
      <div style={{ background: 'rgba(255,255,255,.03)', borderBottom: '1px solid rgba(255,255,255,.06)', padding: '14px 28px', display: 'flex', alignItems: 'center', gap: 16 }}>
        <span style={{ fontSize: 18, fontWeight: 600, color: '#fafafa', letterSpacing: '-0.02em' }}>مدير الميزات</span>
        <div style={{ marginRight: 'auto', display: 'flex', gap: 0, borderBottom: '1px solid rgba(255,255,255,.06)' }}>
      ...
      {/* Stats bar */}
      <div style={{ background: 'rgba(255,255,255,.02)', borderBottom: '1px solid rgba(255,255,255,.04)', padding: '8px 28px', display: 'flex', gap: 24, overflowX: 'auto', alignItems: 'center' }}>
        <div style={{ fontSize: 11, color: '#52525b', whiteSpace: 'nowrap', fontWeight: 500 }}>{totalCompanies} شركة · {featureKeys.length} ميزة</div>
        {featureKeys.slice(0, 8).map(k => {
          ...
          return (
            <div key={k} onClick={() => { setView('feature'); setSelectedFeature(k); loadFeatureCompanies(k); }}
              ...
              <div style={{ width: 32, height: 4, borderRadius: 2, background: 'rgba(255,255,255,.06)' }}>
                <div style={{ height: 4, borderRadius: 2, background: s.adoption_pct > 60 ? '#22c55e' : s.adoption_pct > 30 ? '#f59e0b' : '#52525b', width: `${s.adoption_pct}%` }} />
              </div>
              <span style={{ fontSize: 11, color: '#52525b', fontWeight: 500 }}>{s.name_ar}</span>
              <span style={{ fontSize: 11, color: s.adoption_pct > 60 ? '#22c55e' : '#52525b', fontWeight: 600 }}>{s.adoption_pct}%</span>
            </div>
          );
        })}
        {recentChanges.length > 0 && (
          <div style={{ marginRight: 'auto', fontSize: 11, color: '#52525b', whiteSpace: 'nowrap', fontWeight: 500 }}>
            آخر تغيير: <span style={{ color: '#a1a1aa' }}>{recentChanges[0]?.feature_key}</span> · {timeAgo(recentChanges[0]?.set_at)}
          </div>
        )}
      </div>
      ...
      {view === 'company' && (
        <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', height: 'calc(100vh - 95px)' }}>
          {/* Company sidebar */}
          <div style={{ background: 'rgba(255,255,255,.02)', borderLeft: '1px solid rgba(255,255,255,.06)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '12px 14px', borderBottom: '1px solid rgba(255,255,255,.04)' }}>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث عن شركة..."
                style={{ width: '100%', padding: '7px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,.08)', fontSize: 13, outline: 'none', boxSizing: 'border-box', background: 'rgba(255,255,255,.04)', color: '#fafafa' }} />
                ...
                {['all', ...TIER_ORDER].map(t => (
                  <button key={t} onClick={() => setTierFilter(t)}
                    style={{ fontSize: 11, padding: '2px 8px', borderRadius: 7, border: '1px solid', cursor: 'pointer', fontWeight: 500,
                      background: tierFilter === t ? '#6366f1' : 'rgba(255,255,255,.04)', color: tierFilter === t ? '#fff' : '#a1a1aa',
                      borderColor: tierFilter === t ? '#6366f1' : 'rgba(255,255,255,.08)' }}>
                    {t === 'all' ? 'الكل' : t}
                  </button>
                ))}
              </div>
                ...
                return (
                  <button key={c.id} onClick={() => { setSelectedCompany(c.id); loadCompanyFlags(c.id); }}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', border: 'none',
                      background: selectedCompany === c.id ? 'rgba(99,102,241,.1)' : 'transparent', cursor: 'pointer',
                      borderBottom: '1px solid rgba(255,255,255,.04)', textAlign: 'right' }}>
                    ...
                    <div style={{ flex: 1, overflow: 'hidden' }}>
                      <div style={{ fontSize: 13, fontWeight: 500, color: '#fafafa', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</div>
          ...
          <div style={{ overflowY: 'auto', padding: '20px 24px' }}>
            {!selectedCompany ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60%', color: '#52525b' }}>
              ...
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 600, color: '#fafafa', letterSpacing: '-0.02em' }}>{selectedCo?.name}</div>
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    <span style={{ fontSize: 11, color: '#52525b', fontWeight: 500 }}>تطبيق سريع:</span>
                    {(['basic', 'professional', 'enterprise'] as Tier[]).map(t => {
                      const tc = TIER_COLOR[t];
                      return (
                        <button key={t} onClick={() => bulkApplyTier(t)} disabled={saving}
                          style={{ fontSize: 12, padding: '7px 16px', borderRadius: 7, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', color: '#a1a1aa', cursor: 'pointer', fontWeight: 500 }}>
                          {t}
                        </button>
                      );
                    })}
                        ...
                        {features.map(key => {
                          ...
                          return (
                            <div key={key} className="glass" style={{
                              ...
                              borderColor: enabled ? 'rgba(34,197,94,.25)' : 'rgba(255,255,255,.06)',
                              background: enabled ? 'rgba(34,197,94,.06)' : 'rgba(255,255,255,.03)',
                              transition: 'all .15s',
                            }}>
                              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                                <div style={{ flex: 1 }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                                    <span style={{ fontSize: 13, fontWeight: 600, color: '#fafafa' }}>{stat?.name_ar || key}</span>
                                    ...
                                    {flag?.plan_includes === false && enabled && (
                                      <span style={{ fontSize: 11, padding: '1px 5px', borderRadius: 4, background: 'rgba(245,158,11,.08)', color: '#f59e0b', border: '1px solid rgba(245,158,11,.2)', fontWeight: 500 }}>خارج الخطة</span>
                                    )}
                                  </div>
                                  <div style={{ fontSize: 11, color: '#52525b', fontFamily: 'monospace', fontWeight: 500 }}>{key}</div>
                                  {flag?.set_at && (
                                    <div style={{ fontSize: 11, color: '#3f3f46', marginTop: 3, fontWeight: 500 }}>آخر تعديل: {timeAgo(flag.set_at)}</div>
                                  )}
                                  {flag?.notes && (
                                    <div style={{ fontSize: 11, color: '#a1a1aa', marginTop: 3, background: 'rgba(255,255,255,.04)', padding: '3px 6px', borderRadius: 4 }}>{flag.notes}</div>
                                  )}
                                </div>
                              ...
                              <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center' }}>
                                {enabled && (
                                  <button onClick={() => { setRolloutModal({ companyId: selectedCompany!, featureKey: key, current: rollout }); setRolloutValue(rollout); }}
                                    style={{ fontSize: 11, padding: '2px 8px', borderRadius: 7, border: '1px solid rgba(34,197,94,.25)', background: 'rgba(34,197,94,.06)', color: '#22c55e', cursor: 'pointer', fontWeight: 500 }}>
                                    {rollout}% طرح
                                  </button>
                                )}
                                <button onClick={() => { setNoteModal({ companyId: selectedCompany!, featureKey: key, currentNote: flag?.notes || '' }); setNoteText(flag?.notes || ''); }}
                                  style={{ fontSize: 11, padding: '2px 8px', borderRadius: 7, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.04)', color: '#a1a1aa', cursor: 'pointer', fontWeight: 500 }}>
                                  ملاحظة
                                </button>
                                <div style={{ marginRight: 'auto', fontSize: 11, color: '#52525b', fontWeight: 500 }}>
      ...
      {view === 'feature' && (
        <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', height: 'calc(100vh - 95px)' }}>
          {/* Feature list */}
          <div style={{ background: 'rgba(255,255,255,.02)', borderLeft: '1px solid rgba(255,255,255,.06)', overflowY: 'auto' }}>
            {featuresByTier.map(({ tier, features }) => {
              if (features.length === 0) return null;
              const tc = TIER_COLOR[tier];
              return (
                <div key={tier}>
                  <div style={{ padding: '8px 14px', background: 'rgba(255,255,255,.04)', borderBottom: '1px solid rgba(255,255,255,.04)', fontSize: 11, fontWeight: 600, color: tc.color }}>
                    {tier.toUpperCase()}
                  </div>
                  {features.map(key => {
                    const s = stats[key];
                    ...
                    return (
                      <button key={key} onClick={() => { setSelectedFeature(key); loadFeatureCompanies(key); }}
                        style={{ width: '100%', padding: '10px 14px', border: 'none', background: active ? 'rgba(99,102,241,.1)' : 'transparent', cursor: 'pointer', textAlign: 'right', borderBottom: '1px solid rgba(255,255,255,.04)', display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 13, fontWeight: active ? 600 : 500, color: '#fafafa' }}>{s?.name_ar || key}</div>
                          <div style={{ fontSize: 11, color: '#3f3f46', fontFamily: 'monospace', marginTop: 1, fontWeight: 500 }}>{key}</div>
                        </div>
                        <div style={{ textAlign: 'left' }}>
                          <div style={{ fontSize: 13, fontWeight: 600, color: (s?.adoption_pct ?? 0) > 60 ? '#22c55e' : '#52525b' }}>{s?.adoption_pct ?? 0}%</div>
                          <div style={{ fontSize: 11, color: '#3f3f46', fontWeight: 500 }}>{s?.enabled_count ?? 0} شركة</div>
                        </div>
                      </button>
                    );
                  })}
            ...
            {!selectedFeature ? (
              ...
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  <div>
                    <div style={{ fontSize: 18, fontWeight: 600, color: '#fafafa', letterSpacing: '-0.02em' }}>{stats[selectedFeature]?.name_ar}</div>
                    <div style={{ fontSize: 11, color: '#52525b', fontFamily: 'monospace', fontWeight: 500 }}>{selectedFeature}</div>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => bulkFeatureTier('basic', true)} disabled={saving}
                      style={{ fontSize: 12, padding: '7px 16px', borderRadius: 7, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.04)', color: '#a1a1aa', cursor: 'pointer', fontWeight: 500 }}>
                      تفعيل basic+
                    </button>
                    ...
                    <button onClick={() => bulkFeatureTier('basic', false)} disabled={saving}
                      style={{ fontSize: 12, padding: '7px 16px', borderRadius: 7, background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.25)', color: '#ef4444', cursor: 'pointer', fontWeight: 500 }}>
                      تعطيل الكل
                    </button>
                  </div>
                </div>
                ...
                {/* Adoption bar */}
                <div style={{ background: 'rgba(255,255,255,.06)', borderRadius: 6, height: 6, marginBottom: 20 }}>
                    ...
                    <thead>
                      <tr style={{ background: 'rgba(255,255,255,.04)', borderBottom: '1px solid rgba(255,255,255,.06)' }}>
                        {['الشركة', 'الخطة', 'الحالة', 'نسبة الطرح', 'آخر تعديل', 'ملاحظات', ''].map(h => (
                          <th key={h} style={{ padding: '9px 14px', fontSize: 11, color: '#52525b', fontWeight: 500, textAlign: 'right' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {featureRows.map((row, i) => {
                        ...
                        return (
                          <tr key={row.company_id} style={{ borderBottom: '1px solid rgba(255,255,255,.03)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,.02)' }}>
                            <td style={{ padding: '10px 14px', fontSize: 13, fontWeight: 500, color: '#fafafa' }}>{row.name}</td>
                            ...
                            <td style={{ padding: '10px 14px', fontSize: 13, color: '#a1a1aa' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                <div style={{ width: 50, height: 4, background: 'rgba(255,255,255,.06)', borderRadius: 2 }}>
                                  <div style={{ width: `${row.rollout_pct}%`, height: 4, background: '#6366f1', borderRadius: 2 }} />
                                </div>
                                <span>{row.rollout_pct}%</span>
                              </div>
                            </td>
                            <td style={{ padding: '10px 14px', fontSize: 11, color: '#52525b', fontWeight: 500 }}>{timeAgo(row.set_at)}</td>
                            <td style={{ padding: '10px 14px', fontSize: 11, color: '#3f3f46', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 500 }}>
                              {row.notes || '—'}
                            </td>
                            <td style={{ padding: '10px 14px' }}>
      ...
      {view === 'matrix' && (
        <div style={{ padding: 24, overflowX: 'auto' }}>
          {matrixLoading ? (
            ...
            <>
              <div style={{ fontSize: 13, color: '#a1a1aa', marginBottom: 14, display: 'flex', gap: 16, alignItems: 'center' }}>
                <span>{matrixData.companies.length} شركة x {Object.keys(matrixData.features).length} ميزة</span>
                <span style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#22c55e', display: 'inline-block', boxShadow: '0 0 6px rgba(34,197,94,.4)' }} /> <span style={{ fontSize: 11, color: '#a1a1aa', fontWeight: 500 }}>مُفعَّل</span>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#3f3f46', display: 'inline-block' }} /> <span style={{ fontSize: 11, color: '#a1a1aa', fontWeight: 500 }}>مُعطَّل</span>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#f59e0b', display: 'inline-block', boxShadow: '0 0 6px rgba(245,158,11,.4)' }} /> <span style={{ fontSize: 11, color: '#a1a1aa', fontWeight: 500 }}>طرح جزئي</span>
                </span>
              </div>
              <div style={{ borderRadius: 10, overflow: 'auto' }}>
                <table style={{ borderCollapse: 'collapse', minWidth: 'max-content' }}>
                  <thead>
                    <tr style={{ background: 'rgba(255,255,255,.04)' }}>
                      <th style={{ padding: '10px 16px', fontSize: 11, color: '#52525b', fontWeight: 500, textAlign: 'right', borderBottom: '1px solid rgba(255,255,255,.06)', borderLeft: '1px solid rgba(255,255,255,.06)', position: 'sticky', right: 0, background: 'rgba(255,255,255,.04)', zIndex: 2, minWidth: 160 }}>الشركة / الخطة</th>
                      {Object.entries(matrixData.features).map(([key, meta]: [string, any]) => (
                        <th key={key} style={{ padding: '6px 8px', fontSize: 11, color: '#52525b', fontWeight: 500, textAlign: 'center', borderBottom: '1px solid rgba(255,255,255,.06)', borderLeft: '1px solid rgba(255,255,255,.03)', maxWidth: 80, minWidth: 70 }}>
                          <div style={{ writingMode: 'vertical-lr', transform: 'rotate(180deg)', whiteSpace: 'nowrap', color: TIER_COLOR[meta.tier_min]?.color }}>
                            {meta.name_ar}
                          </div>
                        </th>
                      ))}
                      <th style={{ padding: '8px', fontSize: 11, color: '#52525b', textAlign: 'center', borderBottom: '1px solid rgba(255,255,255,.06)', fontWeight: 500 }}>مجموع</th>
                    </tr>
                  </thead>
                  <tbody>
                    {matrixData.companies.map((company, i) => {
                      ...
                      return (
                        <tr key={company.id} style={{ borderBottom: '1px solid rgba(255,255,255,.03)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,.02)' }}>
                          <td style={{ padding: '8px 14px', borderLeft: '1px solid rgba(255,255,255,.06)', position: 'sticky', right: 0, background: i % 2 === 0 ? '#09090b' : 'rgba(255,255,255,.02)', zIndex: 1 }}>
                            <div style={{ fontSize: 13, fontWeight: 500, color: '#fafafa' }}>{company.name}</div>
                          ...
                          {Object.keys(matrixData.features).map(key => {
                            ...
                            return (
                              <td key={key} style={{ padding: '6px 8px', textAlign: 'center', borderLeft: '1px solid rgba(255,255,255,.03)', cursor: saving ? 'default' : 'pointer' }}
                                onClick={() => toggle(company.id, key, enabled)}>
                                <div style={{
                                  width: 22, height: 22, borderRadius: 6, margin: '0 auto',
                                  background: enabled ? (partial ? '#f59e0b' : '#22c55e') : '#3f3f46',
                                  display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11,
                                  transition: 'background .15s',
                                  color: enabled ? '#fff' : '#52525b',
                          ...
                          <td style={{ padding: '8px', textAlign: 'center' }}>
                            <span style={{ fontSize: 13, fontWeight: 600, color: enabledCount > 8 ? '#22c55e' : enabledCount > 4 ? '#f59e0b' : '#52525b' }}>
                              {enabledCount}
                            </span>
                          </td>
                        </tr>
                    ...
                    {/* Column totals */}
                    <tr style={{ background: 'rgba(255,255,255,.04)', borderTop: '2px solid rgba(255,255,255,.08)' }}>
                      <td style={{ padding: '8px 14px', fontSize: 11, fontWeight: 600, color: '#fafafa', position: 'sticky', right: 0, background: 'rgba(255,255,255,.04)' }}>مجموع الانتشار</td>
                      {Object.keys(matrixData.features).map(key => {
                        const s = stats[key];
                        return (
                          <td key={key} style={{ padding: '6px 8px', textAlign: 'center' }}>
                            <span style={{ fontSize: 11, fontWeight: 600, color: (s?.adoption_pct ?? 0) > 60 ? '#22c55e' : '#52525b' }}>
                              {s?.adoption_pct ?? 0}%
                            </span>
                          </td>
                        );
