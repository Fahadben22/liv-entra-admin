...
export default function FeaturesPage() {
  ...
  return (
    <div className="fade-in" style={{ fontFamily: 'system-ui, sans-serif' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.3)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 24, width: 380, boxShadow: '0 20px 60px rgba(0,0,0,.12)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 16, justifyContent: 'flex-end' }}>
              <button onClick={() => setRolloutModal(null)} style={{ padding: '7px 16px', borderRadius: 10, border: '1px solid rgba(0,0,0,.08)', background: 'transparent', cursor: 'pointer', fontSize: 12, fontWeight: 500, color: '#6b7280' }}>إلغاء</button>
              <button onClick={async () => {
      ...
      {/* Stats bar */}
      <div style={{ borderBottom: '1px solid rgba(0,0,0,.04)', padding: '8px 28px', display: 'flex', gap: 24, overflowX: 'auto', alignItems: 'center' }}>
        <div style={{ fontSize: 11, color: '#6b7280', whiteSpace: 'nowrap', fontWeight: 500 }}>{totalCompanies} شركة · {featureKeys.length} ميزة</div>
          ...
          return (
            <div key={k} onClick={() => { setView('feature'); setSelectedFeature(k); loadFeatureCompanies(k); }}
              ...
              <div style={{ width: 32, height: 4, borderRadius: 2, background: 'rgba(0,0,0,.06)' }}>
                <div style={{ height: 4, borderRadius: 2, background: s.adoption_pct > 60 ? '#22c55e' : s.adoption_pct > 30 ? '#f59e0b' : '#6b7280', width: `${s.adoption_pct}%` }} />
              </div>
              <span style={{ fontSize: 11, color: '#6b7280', fontWeight: 500 }}>{s.name_ar}</span>
              <span style={{ fontSize: 11, color: s.adoption_pct > 60 ? '#22c55e' : '#6b7280', fontWeight: 600 }}>{s.adoption_pct}%</span>
            </div>
          );
        })}
        {recentChanges.length > 0 && (
          <div style={{ marginRight: 'auto', fontSize: 11, color: '#6b7280', whiteSpace: 'nowrap', fontWeight: 500 }}>
            آخر تغيير: <span style={{ color: '#9ca3af' }}>{recentChanges[0]?.feature_key}</span> · {timeAgo(recentChanges[0]?.set_at)}
          </div>
        )}
      </div>
      ...
      {view === 'company' && (
        <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', height: 'calc(100vh - 95px)' }}>
          ...
          <div style={{ background: '#fff', borderLeft: '1px solid rgba(0,0,0,.06)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {filteredCompanies.length === 0 ? (
                <div style={{ padding: 24, textAlign: 'center', color: '#9ca3af', fontSize: 13 }}>لا توجد شركات</div>
                ...
                return (
                  <button key={c.id} onClick={() => { setSelectedCompany(c.id); loadCompanyFlags(c.id); }}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', border: 'none',
                      ...
                      borderBottom: '1px solid rgba(0,0,0,.04)', textAlign: 'right' }}>
          ...
          <div style={{ overflowY: 'auto', padding: '20px 24px' }}>
            {!selectedCompany ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60%', color: '#6b7280' }}>
              ...
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    <span style={{ fontSize: 11, color: '#6b7280', fontWeight: 500 }}>تطبيق سريع:</span>
                        ...
                        {features.map(key => {
                          ...
                          return (
                            <div key={key} className="card" style={{
                              ...
                              borderColor: enabled ? 'rgba(34,197,94,.25)' : 'rgba(0,0,0,.06)',
                              ...
                              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                                <div style={{ flex: 1 }}>
                                  ...
                                  <div style={{ fontSize: 11, color: '#6b7280', fontFamily: 'monospace', fontWeight: 500 }}>{key}</div>
                                  {flag?.set_at && (
                              ...
                              <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center' }}>
                                ...
                                <button onClick={() => { setNoteModal({ companyId: selectedCompany!, featureKey: key, currentNote: flag?.notes || '' }); setNoteText(flag?.notes || ''); }}
                                  style={{ fontSize: 11, padding: '2px 8px', borderRadius: 7, border: '1px solid rgba(0,0,0,.08)', background: 'transparent', color: '#9ca3af', cursor: 'pointer', fontWeight: 500 }}>
                                  ملاحظة
                                </button>
                                <div style={{ marginRight: 'auto', fontSize: 11, color: '#6b7280', fontWeight: 500 }}>
      ...
      {view === 'feature' && (
        <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', height: 'calc(100vh - 95px)' }}>
          {/* Feature list */}
          <div style={{ background: '#fff', borderLeft: '1px solid rgba(0,0,0,.06)', overflowY: 'auto' }}>
            {featuresByTier.map(({ tier, features }) => {
              if (features.length === 0) return null;
              const tc = TIER_COLOR[tier];
              return (
                    ...
                    return (
                      <button key={key} onClick={() => { setSelectedFeature(key); loadFeatureCompanies(key); }}
                        ...
                        <div style={{ textAlign: 'left' }}>
                          <div style={{ fontSize: 13, fontWeight: 600, color: (s?.adoption_pct ?? 0) > 60 ? '#22c55e' : '#6b7280' }}>{s?.adoption_pct ?? 0}%</div>
                          <div style={{ fontSize: 11, color: '#9ca3af', fontWeight: 500 }}>{s?.enabled_count ?? 0} شركة</div>
                        </div>
                      </button>
                    );
                  })}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => bulkFeatureTier('basic', true)} disabled={saving}
                      style={{ fontSize: 12, padding: '7px 16px', borderRadius: 10, border: '1px solid rgba(0,0,0,.08)', background: 'transparent', color: '#6b7280', cursor: 'pointer', fontWeight: 500 }}>
                      تفعيل basic+
                    </button>
                      ...
                      {featureRows.map((row, i) => {
                        ...
                        return (
                          <tr key={row.company_id} style={{ borderBottom: '1px solid rgba(0,0,0,.04)', background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                            ...
                            <td style={{ padding: '10px 14px', fontSize: 11, color: '#6b7280', fontWeight: 500 }}>{timeAgo(row.set_at)}</td>
                            <td style={{ padding: '10px 14px', fontSize: 11, color: '#9ca3af', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 500 }}>
                              {row.notes || '—'}
                            </td>
                            <td style={{ padding: '10px 14px' }}>
      ...
      {view === 'matrix' && (
        <div style={{ padding: 24, overflowX: 'auto' }}>
          {matrixLoading ? (
            <div style={{ textAlign: 'center', padding: 60, color: '#9ca3af', fontSize: 13 }}>جاري تحميل المصفوفة...</div>
          ) : !matrixData ? (
            <div style={{ textAlign: 'center', padding: 60, color: '#9ca3af', fontSize: 13 }}>لا توجد بيانات</div>
          ) : (
            <>
              <div style={{ fontSize: 13, color: '#9ca3af', marginBottom: 14, display: 'flex', gap: 16, alignItems: 'center' }}>
                <span>{matrixData.companies.length} شركة x {Object.keys(matrixData.features).length} ميزة</span>
                <span style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#22c55e', display: 'inline-block', boxShadow: '0 0 6px rgba(34,197,94,.4)' }} /> <span style={{ fontSize: 11, color: '#9ca3af', fontWeight: 500 }}>مُفعَّل</span>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#d1d5db', display: 'inline-block' }} /> <span style={{ fontSize: 11, color: '#9ca3af', fontWeight: 500 }}>مُعطَّل</span>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#f59e0b', display: 'inline-block', boxShadow: '0 0 6px rgba(245,158,11,.4)' }} /> <span style={{ fontSize: 11, color: '#9ca3af', fontWeight: 500 }}>طرح جزئي</span>
                </span>
              </div>
                ...
                <table style={{ borderCollapse: 'collapse', minWidth: 'max-content' }}>
                  <thead>
                    <tr style={{ background: '#F1F5F9' }}>
                      ...
                      <th style={{ padding: '8px', fontSize: 11, color: '#6b7280', textAlign: 'center', borderBottom: '1px solid rgba(0,0,0,.06)', fontWeight: 500 }}>مجموع</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        <tr key={company.id} style={{ borderBottom: '1px solid rgba(0,0,0,.04)', background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                          ...
                          {Object.keys(matrixData.features).map(key => {
                            ...
                            return (
                              <td key={key} style={{ padding: '6px 8px', textAlign: 'center', borderLeft: '1px solid rgba(0,0,0,.04)', cursor: saving ? 'default' : 'pointer' }}
                          ...
                          <td style={{ padding: '8px', textAlign: 'center' }}>
                            <span style={{ fontSize: 13, fontWeight: 600, color: enabledCount > 8 ? '#22c55e' : enabledCount > 4 ? '#f59e0b' : '#6b7280' }}>
                              {enabledCount}
                            </span>
                          </td>
                        </tr>
                      ...
                      {Object.keys(matrixData.features).map(key => {
                        ...
                        return (
                          <td key={key} style={{ padding: '6px 8px', textAlign: 'center' }}>
                            <span style={{ fontSize: 11, fontWeight: 600, color: (s?.adoption_pct ?? 0) > 60 ? '#22c55e' : '#6b7280' }}>
                              {s?.adoption_pct ?? 0}%
                            </span>
                          </td>
                        );
