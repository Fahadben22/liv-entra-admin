...
const ANOMALY_STATUS: Record<string, { color: string; label: string }> = {
  ...
  false_positive: { color: '#52525b', label: 'إيجابي كاذب' },
};
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ background: 'rgba(255,255,255,.03)', borderBottom: '1px solid rgba(255,255,255,.06)', padding: '14px 28px', display: 'flex', alignItems: 'center', gap: 16 }}>
        <span style={{ fontSize: 18, fontWeight: 600, color: '#fafafa', letterSpacing: '-0.02em' }}>التدقيق والأمان</span>
        {openCount > 0 && (
      ...
      <div style={{ background: 'rgba(255,255,255,.03)', borderBottom: '1px solid rgba(255,255,255,.06)', padding: '0 32px', display: 'flex', gap: 0 }}>
        {[
          ...
          <button key={t.k} onClick={() => setTab(t.k as any)}
            style={{ fontSize: 13, padding: '12px 18px', border: 'none', background: 'none', cursor: 'pointer', fontWeight: tab === t.k ? 600 : 400, color: tab === t.k ? '#fafafa' : '#52525b', borderBottom: tab === t.k ? '2px solid #6366f1' : '2px solid transparent', transition: 'all .15s' }}>
            {t.l}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: '28px 32px', maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, alignItems: 'center' }}>
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            style={{ padding: '7px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,.08)', fontSize: 13, outline: 'none', width: 280, background: 'rgba(255,255,255,.04)', color: '#fafafa' }} />
              ...
              {[
                ...
                <button key={f.k} onClick={() => setSevFilter(f.k)}
                  style={{ fontSize: 12, padding: '7px 16px', borderRadius: 7, border: '1px solid', cursor: 'pointer', fontWeight: 500,
                    background: sevFilter === f.k ? '#6366f1' : 'rgba(255,255,255,.04)',
                    color:      sevFilter === f.k ? '#fff'    : '#a1a1aa',
                    borderColor: sevFilter === f.k ? '#6366f1' : 'rgba(255,255,255,.08)' }}>
                  {f.l}
                </button>
              ))}
            </div>
              ...
              <div>
                {filteredAnomalies.length === 0 ? (
                  ...
                  filteredAnomalies.map(a => {
                    ...
                    return (
                      <div key={a.id} className="glass" style={{ background: 'rgba(255,255,255,.03)', borderRadius: 10, border: `1px solid ${a.status === 'open' ? sc.color + '33' : 'rgba(255,255,255,.06)'}`, padding: '16px 20px', marginBottom: 10 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <h4 style={{ fontSize: 13, fontWeight: 600, margin: 0, color: '#fafafa' }}>
                        ...
                        <p style={{ fontSize: 13, color: '#a1a1aa', margin: '0 0 6px', lineHeight: 1.6 }}>{a.description}</p>
                        ...
                        {a.company && (
                          <p style={{ fontSize: 11, color: '#a1a1aa', margin: '0 0 10px', fontWeight: 500 }}>
                            الشركة: <Link href={`/dashboard/companies/${a.company_id}`} style={{ color: '#6366f1', fontWeight: 500 }}>{a.company.name}</Link>
                          </p>
                        )}
                        ...
                        {a.status === 'open' && (
                          <div style={{ display: 'flex', gap: 8 }}>
                            ...
                            <button onClick={() => handleUpdateAnomaly(a.id, 'resolved')} disabled={isActing}
                              style={{ fontSize: 12, padding: '7px 16px', borderRadius: 7, background: '#6366f1', border: 'none', color: '#fff', cursor: 'pointer', fontWeight: 500 }}>
                              حلّ
                            </button>
            ...
            {tab === 'audit' && (
              <>
                <div style={{ borderRadius: 10, overflow: 'hidden' }}>
                  <div style={{ padding: '12px 20px', borderBottom: '1px solid rgba(255,255,255,.04)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,.03)' }}>
                    <span style={{ fontSize: 13, color: '#a1a1aa' }}>{filteredAudit.length} حدث</span>
                    <span style={{ fontSize: 11, color: '#52525b', fontWeight: 500 }}>صفحة {page+1} من {Math.max(1,totalPages)}</span>
                  </div>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ background: 'rgba(255,255,255,.04)' }}>
                        {['الإجراء', 'المشغّل', 'الدور', 'النوع', 'IP', 'التاريخ'].map(h => (
                          <th key={h} style={{ padding: '10px 18px', textAlign: 'right', fontSize: 11, fontWeight: 500, color: '#52525b', borderBottom: '1px solid rgba(255,255,255,.06)' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {paginated.map((a, i) => (
                        <tr key={a.id} style={{ borderBottom: '1px solid rgba(255,255,255,.03)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,.02)' }}>
                          <td style={{ padding: '11px 18px' }}>
                            <span style={{ fontSize: 13, fontWeight: 500, color: '#fafafa' }}>{a.action?.replace(/_/g,' ')}</span>
                          </td>
                          <td style={{ padding: '11px 18px', fontSize: 13, color: '#a1a1aa' }}>{a.actor_email}</td>
                          <td style={{ padding: '11px 18px' }}>
                            <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 7, background: 'rgba(255,255,255,.05)', color: '#a1a1aa', fontWeight: 500, border: '1px solid rgba(255,255,255,.08)' }}>
                              {a.actor_role}
                            </span>
                          </td>
                          <td style={{ padding: '11px 18px', fontSize: 11, color: '#52525b', fontWeight: 500 }}>{a.target_type}</td>
                          <td style={{ padding: '11px 18px', fontSize: 11, color: '#52525b', direction: 'ltr', fontWeight: 500 }}>{a.ip_address}</td>
                          <td style={{ padding: '11px 18px', fontSize: 11, color: '#52525b', direction: 'ltr', whiteSpace: 'nowrap', fontWeight: 500 }}>
                ...
                {totalPages > 1 && (
                  <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 20 }}>
                    <button onClick={() => setPage(p => Math.max(0, p-1))} disabled={page === 0}
                      style={{ padding: '7px 16px', borderRadius: 7, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.04)', cursor: page > 0 ? 'pointer' : 'not-allowed', color: page > 0 ? '#fafafa' : '#52525b', fontSize: 12, fontWeight: 500 }}>
                      السابق
                    </button>
                    <span style={{ padding: '7px 14px', fontSize: 13, color: '#a1a1aa' }}>{page+1} / {totalPages}</span>
                    <button onClick={() => setPage(p => Math.min(totalPages-1, p+1))} disabled={page >= totalPages-1}
                      style={{ padding: '7px 16px', borderRadius: 7, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.04)', cursor: page < totalPages-1 ? 'pointer' : 'not-allowed', color: page < totalPages-1 ? '#fafafa' : '#52525b', fontSize: 12, fontWeight: 500 }}>
                      التالي
                    </button>
                  </div>
                )}
