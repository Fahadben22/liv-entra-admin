  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ borderBottom: '1px solid var(--lv-line)', padding: '14px 28px', display: 'flex', alignItems: 'center', gap: 16 }}>
        <span style={{ fontSize: 18, fontWeight: 600, color: 'var(--lv-fg)', letterSpacing: '-0.02em' }}>التدقيق والأمان</span>
        {openCount > 0 && (
      ...
      <div style={{ borderBottom: '1px solid var(--lv-line)', padding: '0 32px', display: 'flex', gap: 0 }}>
        {[
          ...
          <button key={t.k} onClick={() => setTab(t.k as any)}
            style={{ fontSize: 13, padding: '12px 18px', border: 'none', background: 'none', cursor: 'pointer', fontWeight: tab === t.k ? 600 : 400, color: tab === t.k ? 'var(--lv-fg)' : 'var(--lv-muted)', borderBottom: tab === t.k ? '2px solid var(--lv-accent)' : '2px solid transparent', transition: 'all .15s' }}>
            {t.l}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: '28px 32px', maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, alignItems: 'center' }}>
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            style={{ padding: '7px 12px', borderRadius: 10, border: '1px solid var(--lv-line)', fontSize: 13, outline: 'none', width: 280, background: 'var(--lv-bg)', color: 'var(--lv-fg)' }} />
              ...
              {[
                ...
                <button key={f.k} onClick={() => setSevFilter(f.k)}
                  style={{ fontSize: 12, padding: '7px 16px', borderRadius: 7, border: '1px solid', cursor: 'pointer', fontWeight: 500,
                    background: sevFilter === f.k ? 'var(--lv-accent)' : 'var(--lv-panel)',
                    color:      sevFilter === f.k ? '#fff'    : 'var(--lv-muted)',
                    borderColor: sevFilter === f.k ? 'var(--lv-accent)' : 'var(--lv-line)' }}>
                  {f.l}
                </button>
              ))}
            </div>
              ...
              <div>
                {filteredAnomalies.length === 0 ? (
                  ...
                  filteredAnomalies.map(a => {
                    ...
                    return (
                      <div key={a.id} className="card" style={{ background: 'var(--lv-panel)', borderRadius: 14, border: `1px solid ${a.status === 'open' ? sc.color + '33' : 'var(--lv-line)'}`, padding: '16px 20px', marginBottom: 10, boxShadow: 'var(--lv-shadow-sm)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <h4 style={{ fontSize: 13, fontWeight: 600, margin: 0, color: 'var(--lv-fg)' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            <span style={{ fontSize: 11, color: 'var(--lv-muted)', direction: 'ltr', fontWeight: 500 }}>{new Date(a.created_at).toLocaleString('en-US')}</span>
                          </div>
                        </div>

                        <p style={{ fontSize: 13, color: 'var(--lv-muted)', margin: '0 0 6px', lineHeight: 1.6 }}>{a.description}</p>
                        ...
                        {a.company && (
                          <p style={{ fontSize: 11, color: 'var(--lv-muted)', margin: '0 0 10px', fontWeight: 500 }}>
                            الشركة: <Link href={`/dashboard/companies/${a.company_id}`} style={{ color: 'var(--lv-accent)', fontWeight: 500 }}>{a.company.name}</Link>
                          </p>
                        )}
                        ...
                        {a.status === 'open' && (
                          <div style={{ display: 'flex', gap: 8 }}>
                            <button onClick={() => handleUpdateAnomaly(a.id, 'acknowledged')} disabled={isActing}
                              style={{ fontSize: 12, padding: '7px 16px', borderRadius: 10, background: 'transparent', border: '1px solid var(--lv-line)', color: 'var(--lv-muted)', cursor: 'pointer', fontWeight: 500 }}>
                              {isActing ? '...' : 'اعتراف'}
                            </button>
                            <button onClick={() => handleUpdateAnomaly(a.id, 'resolved')} disabled={isActing}
                              style={{ fontSize: 12, padding: '7px 16px', borderRadius: 10, background: 'var(--lv-accent)', border: 'none', color: '#fff', cursor: 'pointer', fontWeight: 500, boxShadow: '0 2px 8px rgba(124,92,252,.2)' }}>
                              حلّ
                            </button>
            ...
            {tab === 'audit' && (
              <>
                <div style={{ borderRadius: 14, overflow: 'hidden', background: 'var(--lv-panel)', boxShadow: 'var(--lv-shadow-sm)' }}>
                  <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--lv-line)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--lv-bg)' }}>
                    <span style={{ fontSize: 13, color: 'var(--lv-muted)' }}>{filteredAudit.length} حدث</span>
                    <span style={{ fontSize: 11, color: 'var(--lv-muted)', fontWeight: 500 }}>صفحة {page+1} من {Math.max(1,totalPages)}</span>
                  </div>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                      {paginated.map((a, i) => (
                        <tr key={a.id} style={{ borderBottom: '1px solid var(--lv-line)', background: i % 2 === 0 ? 'var(--lv-panel)' : 'var(--lv-bg)' }}>
                          <td style={{ padding: '11px 18px' }}>
                            <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--lv-fg)' }}>{a.action?.replace(/_/g,' ')}</span>
                          </td>
                          <td style={{ padding: '11px 18px', fontSize: 13, color: 'var(--lv-muted)' }}>{a.actor_email}</td>
                          <td style={{ padding: '11px 18px' }}>
                            <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 7, background: 'var(--lv-bg)', color: 'var(--lv-muted)', fontWeight: 500, border: '1px solid var(--lv-line)' }}>
                              {a.actor_role}
                            </span>
                          </td>
                          <td style={{ padding: '11px 18px', fontSize: 11, color: 'var(--lv-muted)', fontWeight: 500 }}>{a.target_type}</td>
                          <td style={{ padding: '11px 18px', fontSize: 11, color: 'var(--lv-muted)', direction: 'ltr', fontWeight: 500 }}>{a.ip_address}</td>
                          <td style={{ padding: '11px 18px', fontSize: 11, color: 'var(--lv-muted)', direction: 'ltr', whiteSpace: 'nowrap', fontWeight: 500 }}>
                            {new Date(a.created_at).toLocaleString('en-US')}
                          </td>
                        </tr>
                      ))}
                      {paginated.length === 0 && (
                        <tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px 0', color: 'var(--lv-muted)', fontSize: 13 }}>لا توجد سجلات</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                ...
                {totalPages > 1 && (
                  <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 20 }}>
                    <button onClick={() => setPage(p => Math.max(0, p-1))} disabled={page === 0}
                      style={{ padding: '7px 16px', borderRadius: 10, border: '1px solid var(--lv-line)', background: 'transparent', cursor: page > 0 ? 'pointer' : 'not-allowed', color: page > 0 ? 'var(--lv-fg)' : 'var(--lv-muted)', fontSize: 12, fontWeight: 500 }}>
                      السابق
                    </button>
                    <span style={{ padding: '7px 14px', fontSize: 13, color: 'var(--lv-muted)' }}>{page+1} / {totalPages}</span>
                    <button onClick={() => setPage(p => Math.min(totalPages-1, p+1))} disabled={page >= totalPages-1}
                      style={{ padding: '7px 16px', borderRadius: 10, border: '1px solid var(--lv-line)', background: 'transparent', cursor: page < totalPages-1 ? 'pointer' : 'not-allowed', color: page < totalPages-1 ? 'var(--lv-fg)' : 'var(--lv-muted)', fontSize: 12, fontWeight: 500 }}>
                      التالي
                    </button>
                  </div>
                )}
