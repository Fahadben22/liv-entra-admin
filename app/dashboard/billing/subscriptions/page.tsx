    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
        {[
          ...
          <div key={s.label} className="card" style={{ background: '#fff', borderRadius: 14, padding: '16px 24px', minWidth: 120, textAlign: 'center', boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
            <p style={{ fontSize: 24, fontWeight: 600, color: s.color, margin: 0 }}>{s.count}</p>
            <p style={{ fontSize: 11, color: '#6b7280', margin: '4px 0 0', fontWeight: 500 }}>{s.label}</p>
          </div>
        ))}
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {safeCompanies.map((c, i) => {
                ...
                return (
                  <tr key={c.id} style={{ borderBottom: i < safeCompanies.length - 1 ? '1px solid rgba(0,0,0,.04)' : 'none', background: i % 2 === 1 ? '#fafafa' : '#fff' }}>
                    <td style={{ padding: '12px 18px' }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: '#1a1a2e', margin: 0 }}>{c.name}</p>
                      {c.name_ar && c.name_ar !== c.name && <p style={{ fontSize: 10, color: '#9ca3af', margin: 0 }}>{c.name_ar}</p>}
                    </td>
                    <td style={{ padding: '12px 18px' }}>
