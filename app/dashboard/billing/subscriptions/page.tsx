...
export default function SubscriptionsPage() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 600, margin: '0 0 20px', color: '#fafafa' }}>الاشتراكات</h2>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
        {[
          ...
          <div key={s.label} style={{ background: 'rgba(255,255,255,.03)', borderRadius: 8, padding: '16px 24px', minWidth: 120, textAlign: 'center', border: '1px solid rgba(255,255,255,.06)' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {safeCompanies.map((c, i) => {
                ...
                return (
                  <tr key={c.id} style={{ borderBottom: i < safeCompanies.length - 1 ? '1px solid rgba(255,255,255,.03)' : 'none', background: i % 2 === 1 ? 'rgba(255,255,255,.02)' : 'transparent' }}>
                    <td style={{ padding: '12px 18px' }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: '#fafafa', margin: 0 }}>{c.name}</p>
                      {c.name_ar && c.name_ar !== c.name && <p style={{ fontSize: 10, color: '#52525b', margin: 0 }}>{c.name_ar}</p>}
                    </td>
                    <td style={{ padding: '12px 18px' }}>
                    ...
                    <td style={{ padding: '12px 18px' }}>
                      <span style={{ fontSize: 11, display: 'inline-flex', alignItems: 'center', gap: 5, color: ss.color, fontWeight: 500 }}>
                        <span style={{ width: 6, height: 6, borderRadius: '50%', background: ss.color, display: 'inline-block', boxShadow: `0 0 6px ${ss.color}` }} />
                        {ss.label}
                      </span>
                    </td>
