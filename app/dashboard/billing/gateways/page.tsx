...
function GatewayCard({ gateway, onSaved }: { gateway: any; onSaved: () => void }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 8, border: '1px solid #e5e5e5', padding: '20px 22px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 7, background: '#fafafa', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, border: '1px solid #f0f0f0', color: '#71717a', fontWeight: 600 }}>
            {gateway.provider.slice(0, 2).toUpperCase()}
          </div>
          <div>
            <p style={{ fontSize: 13, fontWeight: 600, margin: 0, color: '#18181b' }}>{GW_LABELS[gateway.provider] || gateway.provider}</p>
            <p style={{ fontSize: 10, color: '#a1a1aa', margin: '2px 0 0' }}>{gateway.provider}</p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 11, display: 'inline-flex', alignItems: 'center', gap: 5, color: active ? '#16a34a' : '#71717a', fontWeight: 500 }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: active ? '#16a34a' : '#a1a1aa', display: 'inline-block' }} />
            {active ? 'مفعّل' : 'معطّل'}
          </span>
      ...
      {expanded && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 12, color: '#18181b' }}>
          ...
          {fields.map(f => (
            <div key={f.key} style={{ marginBottom: 10 }}>
              ...
              <input value={keys[f.key] || ''} onChange={e => setKey(f.key, e.target.value)} placeholder={f.placeholder}
                type="password"
                style={{ width: '100%', padding: '7px 10px', borderRadius: 7, border: '1px solid #e5e5e5', fontSize: 13, fontFamily: 'monospace', boxSizing: 'border-box' }} />
            </div>
          ))}
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            <button onClick={handleSave} disabled={saving}
              style={{ flex: 1, padding: '9px', borderRadius: 7, border: 'none', background: '#18181b', color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
              {saving ? 'جاري الحفظ...' : 'حفظ الإعدادات'}
            </button>
            <button onClick={handleTest} disabled={testing || !active}
              style={{ padding: '9px 16px', borderRadius: 7, border: '1px solid #e5e5e5', background: '#fff', cursor: 'pointer', fontSize: 13, color: '#71717a' }}>
              {testing ? '...' : 'اختبار'}
            </button>
          </div>
        </>
...
export default function GatewaysPage() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 600, margin: '0 0 16px', color: '#18181b' }}>بوابات الدفع</h2>

      <div style={{ background: '#fff', borderRadius: 8, border: '1px solid #e5e5e5', padding: '14px 18px', fontSize: 12, color: '#71717a', lineHeight: 1.6, marginBottom: 20 }}>
        Tap Payments هي البوابة الرئيسية للسوق السعودي. أدخل مفاتيح API وفعّل البوابة لبدء استقبال المدفوعات.
        <br />Webhook URL: <code style={{ fontSize: 11, background: '#fafafa', padding: '1px 5px', borderRadius: 4, border: '1px solid #f0f0f0' }}>/api/v1/billing/webhook/tap</code>
      </div>
