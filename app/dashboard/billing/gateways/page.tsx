...
function GatewayCard({ gateway, onSaved }: { gateway: any; onSaved: () => void }) {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,.03)', borderRadius: 8, border: '1px solid rgba(255,255,255,.06)', padding: '20px 22px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 7, background: 'rgba(255,255,255,.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, border: '1px solid rgba(255,255,255,.06)', color: '#a1a1aa', fontWeight: 600 }}>
            {gateway.provider.slice(0, 2).toUpperCase()}
          </div>
          <div>
            <p style={{ fontSize: 13, fontWeight: 600, margin: 0, color: '#fafafa' }}>{GW_LABELS[gateway.provider] || gateway.provider}</p>
            <p style={{ fontSize: 10, color: '#52525b', margin: '2px 0 0' }}>{gateway.provider}</p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 11, display: 'inline-flex', alignItems: 'center', gap: 5, color: active ? '#16a34a' : '#52525b', fontWeight: 500 }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: active ? '#16a34a' : '#52525b', display: 'inline-block', boxShadow: active ? '0 0 6px #16a34a' : 'none' }} />
            {active ? 'مفعّل' : 'معطّل'}
          </span>
          <button onClick={() => setExpanded(e => !e)}
            style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, color: '#a1a1aa' }}>
            {expanded ? '▲' : '▼'}
          </button>
        </div>
      </div>
      ...
      {expanded && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 12, color: '#fafafa' }}>
          ...
          {fields.map(f => (
            <div key={f.key} style={{ marginBottom: 10 }}>
              <label style={{ fontSize: 11, fontWeight: 500, display: 'block', marginBottom: 4, color: '#a1a1aa' }}>{f.label}</label>
              <input value={keys[f.key] || ''} onChange={e => setKey(f.key, e.target.value)} placeholder={f.placeholder}
                type="password"
                style={{ width: '100%', padding: '7px 10px', borderRadius: 7, border: '1px solid rgba(255,255,255,.08)', fontSize: 13, fontFamily: 'monospace', boxSizing: 'border-box', background: 'rgba(255,255,255,.04)', color: '#fafafa' }} />
            </div>
          ))}
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            <button onClick={handleSave} disabled={saving}
              style={{ flex: 1, padding: '9px', borderRadius: 7, border: 'none', background: '#6366f1', color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
              {saving ? 'جاري الحفظ...' : 'حفظ الإعدادات'}
            </button>
            <button onClick={handleTest} disabled={testing || !active}
              style={{ padding: '9px 16px', borderRadius: 7, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.04)', cursor: 'pointer', fontSize: 13, color: '#a1a1aa' }}>
              {testing ? '...' : 'اختبار'}
            </button>
          </div>
        </>
...
export default function GatewaysPage() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 600, margin: '0 0 16px', color: '#fafafa' }}>بوابات الدفع</h2>

      <div style={{ background: 'rgba(255,255,255,.03)', borderRadius: 8, border: '1px solid rgba(255,255,255,.06)', padding: '14px 18px', fontSize: 12, color: '#a1a1aa', lineHeight: 1.6, marginBottom: 20 }}>
        Tap Payments هي البوابة الرئيسية للسوق السعودي. أدخل مفاتيح API وفعّل البوابة لبدء استقبال المدفوعات.
        <br />Webhook URL: <code style={{ fontSize: 11, background: 'rgba(255,255,255,.04)', padding: '1px 5px', borderRadius: 4, border: '1px solid rgba(255,255,255,.06)', color: '#a1a1aa' }}>/api/v1/billing/webhook/tap</code>
      </div>
