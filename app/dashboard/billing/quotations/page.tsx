...
export default function QuotationsPage() {
  ...
  const ghostBtn: React.CSSProperties = { fontSize: 10, padding: '5px 8px', borderRadius: 7, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', textDecoration: 'none' };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h2 style={{ fontSize: 18, fontWeight: 600, margin: 0, color: '#fafafa' }}>عروض الأسعار</h2>
        <button onClick={() => setShowCreate(true)} style={{ fontSize: 13, padding: '8px 18px', borderRadius: 7, background: '#6366f1', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
          + عرض سعر جديد
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 80, color: '#a1a1aa', background: 'rgba(255,255,255,.03)', borderRadius: 8, border: '1px solid rgba(255,255,255,.06)' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {items.map((q, i) => {
                ...
                return (
                  <tr key={q.id} style={{ borderBottom: i < items.length - 1 ? '1px solid rgba(255,255,255,.03)' : 'none', background: i % 2 === 1 ? 'rgba(255,255,255,.02)' : 'transparent' }}>
                    <td style={{ padding: '12px 18px', fontSize: 12, fontWeight: 600, fontFamily: 'monospace', color: '#fafafa' }}>{q.quotation_number}</td>
                    <td style={{ padding: '12px 18px', fontSize: 13, color: '#fafafa' }}>{q.company?.name || q.company?.name_ar || '—'}</td>
                    <td style={{ padding: '12px 18px', fontSize: 13, fontWeight: 600, color: '#fafafa' }}>{Number(q.total_sar || 0).toLocaleString()} ر.س</td>
                    ...
                    <td style={{ padding: '12px 18px', fontSize: 12, color: '#52525b' }}>{q.valid_until || '—'}</td>
                    <td style={{ padding: '12px 18px' }}>
...
function CreateQuotationModal({ companies, onClose, onDone }: { companies: any[]; onClose: () => void; onDone: () => void }) {
  ...
  const inputStyle: React.CSSProperties = { width: '100%', padding: '9px 12px', borderRadius: 7, border: '1px solid rgba(255,255,255,.08)', fontSize: 13, fontFamily: 'inherit', boxSizing: 'border-box', outline: 'none', background: 'rgba(255,255,255,.04)', color: '#fafafa' };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.6)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ background: '#18181b', border: '1px solid rgba(255,255,255,.1)', borderRadius: 8, padding: '32px 28px', width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <h3 style={{ fontSize: 13, fontWeight: 600, margin: '0 0 20px', color: '#fafafa' }}>عرض سعر جديد</h3>
        ...
        {items.map((item, i) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 100px 30px', gap: 8, marginBottom: 8 }}>
            ...
            {items.length > 1 && (
              <button onClick={() => removeItem(i)} style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', color: '#dc2626', borderRadius: 7, cursor: 'pointer', fontSize: 14 }}>×</button>
            )}
          </div>
        ))}
        <button onClick={addItem} style={{ fontSize: 12, color: '#a1a1aa', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', marginBottom: 16 }}>+ إضافة بند</button>
        ...
        {subtotal > 0 && (
          <div style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)', borderRadius: 8, padding: '12px 16px', marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
              ...
              <span style={{ color: '#fafafa' }}>{subtotal.toLocaleString()} ر.س</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
              <span style={{ color: '#a1a1aa' }}>ضريبة 15%</span>
              <span style={{ color: '#fafafa' }}>{vat.toLocaleString(undefined, { minimumFractionDigits: 2 })} ر.س</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, fontWeight: 600, borderTop: '1px solid rgba(255,255,255,.06)', paddingTop: 6 }}>
              <span style={{ color: '#fafafa' }}>الإجمالي</span>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ padding: '10px 20px', borderRadius: 7, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.04)', color: '#a1a1aa', cursor: 'pointer', fontSize: 13, fontFamily: 'inherit' }}>إلغاء</button>
          <button onClick={submit} disabled={loading} style={{ padding: '10px 24px', borderRadius: 7, border: 'none', background: '#6366f1', color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'inherit', opacity: loading ? .7 : 1 }}>
