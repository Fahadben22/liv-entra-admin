  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ background: '#fff', borderRadius: 16, padding: '28px 32px', width: 380, direction: 'rtl' }} onClick={e => e.stopPropagation()}>
        ...
        <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 20px' }}>{invoice.invoice_number} — {fmt(invoice.total_sar)} ر.س</p>
...
function CompanyDrawer({ companyId, onClose }: { companyId: string; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', zIndex: 200, display: 'flex', justifyContent: 'flex-end' }} onClick={onClose}>
      <div style={{ width: 480, background: '#fff', height: '100%', overflowY: 'auto', padding: '28px', direction: 'rtl' }} onClick={e => e.stopPropagation()}>
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h2 style={{ fontSize: 16, fontWeight: 700, margin: 0 }}>الفواتير</h2>
          ...
          {(['all', 'issued', 'overdue', 'paid', 'waived', 'void'] as const).map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              style={{
                ...
                border: `1px solid ${statusFilter === s ? '#1d4070' : '#e2e8f0'}`,
                background: statusFilter === s ? '#1d4070' : '#fff',
                color: statusFilter === s ? '#fff' : '#64748b',
                fontSize: 11, cursor: 'pointer', fontWeight: statusFilter === s ? 700 : 400,
              }}>
                  ...
                  <tr key={inv.id} style={{ borderBottom: i < filteredInvoices.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                    ...
                    <td style={{ padding: '12px 18px' }}>
                      <button onClick={() => setDrawerCompId(inv.company_id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'right' }}>
                        <p style={{ fontSize: 13, fontWeight: 600, color: '#0f172a', margin: 0 }}>{companyName}</p>
                      </button>
                    </td>
                    <td style={{ padding: '12px 18px' }}>
                      <p style={{ fontSize: 14, fontWeight: 700, color: '#0f172a', margin: 0 }}>{fmt(inv.total_sar)} ر.س</p>
                      {Number(inv.vat_sar) > 0 && <p style={{ fontSize: 10, color: '#94a3b8', margin: 0 }}>شامل {fmt(inv.vat_sar)} ض.ق.م</p>}
                    </td>
                    <td style={{ padding: '12px 18px' }}>
