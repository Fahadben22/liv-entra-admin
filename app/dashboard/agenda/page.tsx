...
export default function AgendaPage() {
  ...
  async function decide(id: string, action: 'approve' | 'reject' | 'dismiss') {
    setBusy(id + action);
    try {
      await request('POST', `/admin/agenda/${id}/${action}`, { note: noteInputs[id] || undefined });
      setItems(prev => prev.map(i => i.id === id
        ? { ...i, status: action === 'approve' ? 'approved' : action === 'reject' ? 'rejected' : 'dismissed', decision_note: noteInputs[id] || null }
        : i
      ));
      setExpanded(null);
    } catch {}
  ...
  function AgentAvatar({ agentType, agentName }: { agentType: string; agentName: string }) {
  ...
  function ItemCard({ item }: { item: AgendaItem }) {
    ...
    return (
      <div style={{
        ...
        >
          <AgentAvatar agentType={item.from_agent} agentName={item.agent_name} />
        ...
        {/* Expanded body */}
        {isExpanded && (
          <div style={{ padding: '0 18px 16px', borderTop: '1px solid rgba(0,0,0,.05)', paddingTop: 14 }}>
            <p style={{ fontSize: 13, color: '#374151', lineHeight: 1.7, whiteSpace: 'pre-wrap', margin: '0 0 14px' }}>
              {item.message}
            </p>
            ...
            {isPending && (
              <>
                <textarea
                  ...
                  rows={2}
                  style={{
                    width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid rgba(0,0,0,.08)',
                    fontSize: 12, lineHeight: 1.6, resize: 'none', boxSizing: 'border-box',
                    direction: 'rtl', marginBottom: 10, background: '#fafafa',
                  }}
                />
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); decide(item.id, 'dismiss'); }}
                    disabled={!!busy}
    ...
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 20px', direction: 'rtl' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: '#9ca3af', margin: '4px 0 0' }}>التصعيدات والقرارات المطلوبة منك</p>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button onClick={load} style={{ fontSize: 11, padding: '6px 14px', borderRadius: 8, border: '1px solid rgba(0,0,0,.08)', background: '#fff', color: '#6b7280', cursor: 'pointer' }}>
            تحديث
          </button>
        </div>
      </div>
      ...
      {!loading && pending.length > 0 && (
        <>
          <p style={{ fontSize: 11, fontWeight: 700, color: '#dc2626', marginBottom: 10, textTransform: 'uppercase', letterSpacing: 1 }}>
      ...
      {!loading && filter === 'all' && decided.length > 0 && (
        <>
          <p style={{ fontSize: 11, fontWeight: 700, color: '#9ca3af', marginBottom: 10, marginTop: 24, textTransform: 'uppercase', letterSpacing: 1 }}>
