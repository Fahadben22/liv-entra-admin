...
export default function NewCompanyPage() {
      ...
      <div style={{ background: '#fff', minHeight: '100vh' }}>
        ...
        <div style={{ padding: 32, maxWidth: 560, margin: '0 auto' }}>
          <div className="card" style={{ background: '#fff', borderRadius: 14, padding: 36, boxShadow: '0 1px 3px rgba(0,0,0,.06)', textAlign: 'center' }}>
            ...
            <div style={{ background: '#F1F5F9', border: '1px solid rgba(0,0,0,.06)', borderRadius: 10, padding: '18px 22px', textAlign: 'right', marginBottom: 24 }}>
              ...
              {[
                ...
                <div key={k as string} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid rgba(0,0,0,.04)' }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, color: '#1E293B', direction: 'ltr', fontFamily: 'monospace' }}>{v as string}</span>
                </div>
              ))}
            </div>
            <div style={{ background: 'rgba(124,92,252,.06)', border: '1px solid rgba(124,92,252,.15)', borderRadius: 10, padding: '10px 14px', textAlign: 'right', marginBottom: 24, fontSize: 12, color: '#2563EB' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => { setSuccess(null); setStep(1); setForm({ country_code: 'SA', name: '', name_ar: '', slug: '', city: 'الرياض', cr_number: '', contact_phone: '', contact_email: '', plan: 'trial', max_units: 50, max_staff: 5, trial_days: 30, billing_cycle: 'monthly', admin_name: '', admin_phone: '', admin_email: '' }); setSlugManual(false); }}
                style={{ flex: 1, padding: '11px', border: '1px solid rgba(0,0,0,.08)', borderRadius: 10, fontSize: 12, cursor: 'pointer', background: '#F1F5F9', color: '#6b7280', fontWeight: 500 }}>
                إضافة شركة أخرى
              </button>
              <button onClick={() => router.push('/dashboard/companies')}
                style={{ flex: 2, padding: '11px', background: '#2563EB', color: '#fff', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                عرض جميع الشركات
              </button>
            </div>
          </div>
  ...
  return (
    <div style={{ background: '#fff', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#F1F5F9', borderBottom: '1px solid rgba(0,0,0,.06)', padding: '0 24px' }}>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={s.num} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '16px 8px 12px', position: 'relative', cursor: done ? 'pointer' : 'default' }}
                ...
                {/* Connector line */}
                {i < STEPS.length - 1 && (
                  <div style={{ position: 'absolute', top: 24, left: 0, width: '50%', height: 1, background: done || step > s.num ? '#2563EB' : 'rgba(0,0,0,.08)', zIndex: 0 }} />
                )}
                {i > 0 && (
                  <div style={{ position: 'absolute', top: 24, right: 0, width: '50%', height: 1, background: done ? '#2563EB' : 'rgba(0,0,0,.08)', zIndex: 0 }} />
                )}
                {/* Dot */}
                <div style={{
                  width: 20, height: 20, borderRadius: '50%',
                  background: active ? '#2563EB' : done ? '#2563EB' : 'transparent',
                  color: active || done ? '#fff' : '#9ca3af',
                  border: active || done ? '2px solid #2563EB' : '2px solid rgba(0,0,0,.12)',
      ...
      <div style={{ padding: '28px 20px', maxWidth: 700, margin: '0 auto' }}>
              ...
              <div style={{ gridColumn: 'span 2' }}>
                <Field label="الدولة" hint="تحدد العملة والضريبة والمنطقة الزمنية تلقائياً">
                  ...
                  {(() => {
                    ...
                    return cc ? (
                      <div style={{ marginTop: 8, padding: '6px 12px', background: '#F1F5F9', borderRadius: 8, fontSize: 11, color: '#6b7280', display: 'flex', gap: 16, border: '1px solid rgba(0,0,0,.06)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    {slugManual && (
                      <button type="button" onClick={() => { setSlugManual(false); setForm(p => ({ ...p, slug: toSlug(form.name) })); }}
                        style={{ padding: '7px 12px', border: '1px solid rgba(0,0,0,.08)', borderRadius: 10, fontSize: 11, cursor: 'pointer', background: '#F1F5F9', color: '#6b7280', whiteSpace: 'nowrap' }}>
                        إعادة توليد
                      </button>
                    )}
                  </div>
                  {form.slug && (
                    <div style={{ marginTop: 6, padding: '6px 10px', background: slugAvailable === false ? 'rgba(239,68,68,.08)' : '#F1F5F9', borderRadius: 10, fontSize: 11, color: slugAvailable === false ? '#dc2626' : '#1E293B', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid rgba(0,0,0,.06)' }}>
        ...
        {step === 2 && (
          <div className="card" style={{ background: '#fff', borderRadius: 14, padding: 28, boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
              ...
              {PLANS.map(p => {
                ...
                return (
                  <div key={p.key} onClick={() => handlePlanChange(p.key)}
                    style={{ border: `1.5px solid ${active ? '#2563EB' : 'rgba(0,0,0,.08)'}`, borderRadius: 14, padding: '16px 18px', cursor: 'pointer', background: active ? 'rgba(124,92,252,.05)' : '#fff', transition: 'all 0.15s' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      <span style={{ fontSize: 13, fontWeight: 600, color: active ? '#1E293B' : '#6b7280' }}>{p.label}</span>
                      <span style={{ fontSize: 10, color: '#9ca3af', background: '#F1F5F9', padding: '2px 8px', borderRadius: 7, border: '1px solid rgba(0,0,0,.06)' }}>{p.desc}</span>
                    </div>
                    <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
                      {p.features.map(f => (
                        <li key={f} style={{ fontSize: 11, color: '#9ca3af', padding: '2px 0', display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ width: 4, height: 4, borderRadius: '50%', background: active ? '#2563EB' : '#9ca3af', display: 'inline-block' }} /> {f}
                        </li>
                      ))}
                    </ul>
            ...
            <div style={{ border: '1px solid rgba(0,0,0,.06)', borderRadius: 14, padding: '18px 20px', marginBottom: 4 }}>
              <p style={{ fontSize: 13, fontWeight: 600, color: '#1E293B', margin: '0 0 14px' }}>حدود الاستخدام</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        {step === 3 && (
          <div className="card" style={{ background: '#fff', borderRadius: 14, padding: 28, boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
            ...
            {/* Phone-login callout */}
            <div style={{ background: '#F1F5F9', border: '1px solid rgba(0,0,0,.06)', borderRadius: 10, padding: '12px 16px', marginBottom: 22 }}>
              <p style={{ fontSize: 12, fontWeight: 600, color: '#1E293B', margin: '0 0 2px' }}>رقم الجوال هو اسم المستخدم</p>
              ...
              <div style={{ gridColumn: 'span 2' }}>
                <Field label="رقم الجوال * (اسم المستخدم)" hint="يبدأ بـ 05 أو +966 أو 966">
                  <input value={form.admin_phone} onChange={set('admin_phone')} placeholder="0512345678" dir="ltr"
                    style={{ ...inp(), border: '1.5px solid #2563EB' }} />
                </Field>
              </div>
              <div style={{ gridColumn: 'span 2' }}>
        ...
        {step === 4 && (
          <div className="card" style={{ background: '#fff', borderRadius: 14, padding: 28, boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={back} style={{ flex: 1, padding: '12px', border: '1px solid rgba(0,0,0,.08)', borderRadius: 10, fontSize: 13, cursor: 'pointer', background: '#F1F5F9', color: '#6b7280', fontWeight: 500 }}>
                ← رجوع
              </button>
              <button onClick={handleSubmit} disabled={saving}
                style={{ flex: 3, padding: '12px', background: saving ? '#9ca3af' : '#2563EB', color: '#fff', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
...
function Header() {
  return (
    <div style={{ background: '#fff', padding: '12px 24px', display: 'flex', alignItems: 'center', gap: 16, borderBottom: '1px solid rgba(0,0,0,.06)' }}>
      ...
      <span style={{ fontSize: 14, fontWeight: 600, color: '#1E293B' }}>إضافة شركة جديدة</span>
    </div>
  );
}
...
function StepHeader({ title, sub }: { title: string; sub: string }) {
  return (
    <div style={{ marginBottom: 24 }}>
      <h2 style={{ fontSize: 13, fontWeight: 600, color: '#1E293B', margin: '0 0 4px' }}>{title}</h2>
...
function inp(extra?: React.CSSProperties): React.CSSProperties {
  return { width: '100%', padding: '9px 11px', border: '1px solid rgba(0,0,0,.08)', borderRadius: 10, fontSize: 13, outline: 'none', boxSizing: 'border-box', background: '#F1F5F9', color: '#1E293B', ...extra };
}
...
function NavButtons({ onBack, onNext }: { onBack?: () => void; onNext?: () => void }) {
  return (
    <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
      {onBack && (
        <button type="button" onClick={onBack} style={{ flex: 1, padding: '11px', border: '1px solid rgba(0,0,0,.08)', borderRadius: 10, fontSize: 13, cursor: 'pointer', background: '#F1F5F9', color: '#6b7280', fontWeight: 500 }}>
          ← رجوع
        </button>
      )}
      {!onBack && <div style={{ flex: 1 }} />}
      {onNext && (
        <button type="button" onClick={onNext} style={{ flex: 2, padding: '11px', background: '#2563EB', color: '#fff', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          التالي ←
        </button>
      )}
    </div>
  ...
  return (
    <div style={{ border: '1px solid rgba(0,0,0,.06)', borderRadius: 14, padding: '16px 18px', position: 'relative', ...style }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <button onClick={onEdit} style={{ fontSize: 10, color: '#6b7280', background: '#F1F5F9', border: '1px solid rgba(0,0,0,.08)', borderRadius: 7, padding: '3px 10px', cursor: 'pointer' }}>تعديل</button>
      </div>
      {rows.map(([k, v]) => (
        <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid rgba(0,0,0,.04)' }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, color: '#1E293B', direction: 'ltr', maxWidth: '60%', textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v}</span>
        </div>
      ))}
    </div>
  );
