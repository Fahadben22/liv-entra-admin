  ...
  return (
    <div style={{ background: '#fafafa' }}>
      ...
      <div style={{ background: '#fff', padding: '14px 28px', display: 'flex', alignItems: 'center', gap: 16, borderBottom: '1px solid #e5e5e5' }}>
        ...
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#18181b' }}>قائمة الإرسال</div>
          <div style={{ fontSize: 11, color: '#a1a1aa' }}>إشعارات واتساب — جميع الشركات</div>
        </div>
      ...
      <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
          {[
            ...
            { label: 'نسبة التسليم', value: `${stats.deliveryRate}%`, color: '#18181b' },
            { label: 'فشل', value: stats.failedToday, color: '#dc2626' },
            { label: 'معلق', value: stats.pending, color: '#d97706' },
          ].map(card => (
            <div key={card.label} style={{ background: '#fff', borderRadius: 8, padding: '16px 20px', border: '1px solid #e5e5e5' }}>
        ...
        {/* Filters */}
        <div style={{ background: '#fff', borderRadius: 8, padding: '12px 20px', marginBottom: 16, display: 'flex', gap: 8, alignItems: 'center', border: '1px solid #e5e5e5' }}>
          ...
          <button onClick={loadQueue} style={{ marginRight: 'auto', padding: '4px 14px', borderRadius: 7, border: '1px solid #e5e5e5', fontSize: 12, cursor: 'pointer', background: '#fafafa', color: '#71717a' }}>
            تحديث
          </button>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {jobs.map((job, i) => {
                  ...
                  return (
                    <tr key={job.id} style={{ borderBottom: '1px solid #f0f0f0', background: i % 2 === 1 ? '#fafafa' : '#fff' }}>
                      <td style={{ padding: '10px 14px', fontSize: 12, color: '#18181b' }}>
                        {job.notification_type.replace(/_/g, ' ')}
                      </td>
                      <td style={{ padding: '10px 14px', fontSize: 12, fontFamily: 'monospace', color: '#18181b' }}>{job.recipient_phone}</td>
                      <td style={{ padding: '10px 14px', fontSize: 12, color: '#71717a' }}>{job.template_name || '—'}</td>
                      ...
                      <td style={{ padding: '10px 14px', fontSize: 12, textAlign: 'center', color: '#71717a' }}>{job.attempts}</td>
                      ...
                      <td style={{ padding: '10px 14px' }}>
                        {job.status === 'failed' && (
                          <button onClick={() => retry(job.id)} disabled={retrying === job.id}
                            style={{ background: '#fafafa', border: '1px solid #e5e5e5', color: '#d97706', fontSize: 11, padding: '3px 10px', borderRadius: 7, cursor: 'pointer' }}>
