...
export default function WhatsAppSettingsPage() {
  ...
  const inputStyle: React.CSSProperties = { width: '100%', border: '1px solid rgba(255,255,255,.08)', borderRadius: 7, padding: '8px 12px', fontSize: 13, boxSizing: 'border-box', color: '#fafafa', background: 'rgba(255,255,255,.04)' };
  ...
  return (
    <div style={{ background: '#09090b' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,.03)', padding: '14px 28px', display: 'flex', alignItems: 'center', gap: 16, borderBottom: '1px solid rgba(255,255,255,.06)', backdropFilter: 'blur(12px)' }}>
        ...
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#fafafa' }}>إعدادات واتساب</div>
          <div style={{ fontSize: 11, color: '#52525b' }}>تكوين بيانات اعتماد واتساب لكل شركة</div>
        </div>
      ...
      <div style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
        {/* Company selector */}
        <div style={{ background: 'rgba(255,255,255,.03)', borderRadius: 8, padding: '16px 20px', marginBottom: 20, border: '1px solid rgba(255,255,255,.06)' }}>
        ...
        {/* Status card */}
        <div style={{ background: 'rgba(255,255,255,.03)', borderRadius: 8, padding: '16px 20px', marginBottom: 20, border: '1px solid rgba(255,255,255,.06)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 10, height: 10, borderRadius: '50%', background: form.wa_setup_complete ? '#16a34a' : '#dc2626', boxShadow: form.wa_setup_complete ? '0 0 6px #16a34a' : '0 0 6px #dc2626' }} />
          <div>
        ...
        {/* Setup form */}
        <div style={{ background: 'rgba(255,255,255,.03)', borderRadius: 8, padding: '20px 24px', marginBottom: 20, border: '1px solid rgba(255,255,255,.06)' }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#fafafa', marginBottom: 16 }}>بيانات اعتماد Meta Business</div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
            <button onClick={save} disabled={saving}
              style={{ background: '#6366f1', color: '#fff', border: 'none', borderRadius: 7, padding: '9px 22px', fontSize: 13, cursor: saving ? 'default' : 'pointer', opacity: saving ? 0.7 : 1, fontWeight: 600 }}>
              {saving ? 'جاري الحفظ...' : 'حفظ الإعدادات'}
            </button>
            <button onClick={testConnection} disabled={testing || !form.wa_phone_number_id}
              style={{ background: 'rgba(255,255,255,.04)', color: '#a1a1aa', border: '1px solid rgba(255,255,255,.08)', borderRadius: 7, padding: '9px 22px', fontSize: 13, cursor: testing ? 'default' : 'pointer', opacity: testing || !form.wa_phone_number_id ? 0.6 : 1 }}>
          ...
          {testResult && (
            <div style={{ marginTop: 10, fontSize: 13, color: testResult.ok ? '#16a34a' : '#dc2626', background: 'rgba(255,255,255,.04)', padding: '8px 12px', borderRadius: 7, border: '1px solid rgba(255,255,255,.06)' }}>
              {testResult.msg}
            </div>
          )}
        </div>
        ...
        {/* Webhook URL */}
        <div style={{ background: 'rgba(255,255,255,.03)', borderRadius: 8, padding: '20px 24px', border: '1px solid rgba(255,255,255,.06)' }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#fafafa', marginBottom: 16 }}>إعدادات Webhook في Meta Business Manager</div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <input value={WEBHOOK_URL} readOnly
                style={{ flex: 1, border: '1px solid rgba(255,255,255,.08)', borderRadius: 7, padding: '8px 12px', fontSize: 12, background: 'rgba(255,255,255,.04)', color: '#52525b', fontFamily: 'monospace' }} />
              <button onClick={() => copy(WEBHOOK_URL, 'url')}
                style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 7, padding: '8px 14px', fontSize: 12, cursor: 'pointer', color: copied === 'url' ? '#16a34a' : '#a1a1aa' }}>
                {copied === 'url' ? 'نُسخ' : 'نسخ'}
              </button>
            </div>
          </div>
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => copy(VERIFY_TOKEN, 'token')}
                style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 7, padding: '8px 14px', fontSize: 12, cursor: 'pointer', color: copied === 'token' ? '#16a34a' : '#a1a1aa' }}>
                {copied === 'token' ? 'نُسخ' : 'نسخ'}
              </button>
            </div>
          </div>

          <div style={{ marginTop: 14, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)', borderRadius: 7, padding: '10px 14px', fontSize: 12, color: '#a1a1aa' }}>
